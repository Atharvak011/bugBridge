...
//       try {
//         let user_id = 14;
//         const response = await axios.get(`http://localhost:8080/api/bugs/allBugs?user_id=${user_id}`);
...
const Dashboard = () => {
  ...
  const fetchBugs = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:8080/api/bugs/allBugs?user_id=2`);
      setBugs(response.data.bugList || []);
      setError(null);
    } catch (err) {
      setError("Failed to fetch bugs");
